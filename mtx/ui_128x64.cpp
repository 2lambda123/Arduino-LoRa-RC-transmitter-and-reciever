...
const char *const srcNames[] PROGMEM = {
    ...
    srcName24, srcName25, srcName26, srcName27, srcName28};
...
void handleMainUI() {
  ...
  case MODE_TIMER_SETUP: {
    ...
    if (focusedItem == 1) {
      Model.timer1ControlSrc = incDecOnUpDown(
          Model.timer1ControlSrc, 0, NUM_MIXSOURCES - 1, NOWRAP, SLOW_CHANGE);
      ...
      Model.timer1Value =
          incDecOnUpDown(Model.timer1Value, -100, 100, NOWRAP, PRESSED_OR_HELD);
    else if (focusedItem == 4)
      Model.timer1InitMins =
          incDecOnUpDown(Model.timer1InitMins, 0, 240, NOWRAP, PRESSED_OR_HELD);
  ...
  case MAIN_MENU: {
    ...
    if (menuHighlightedItem < menuTopItem ||
      ...
      menuTopItem =
          incDecOnUpDown(menuTopItem, _numMenuItems - 3, 1, WRAP, SLOW_CHANGE);
    isEditMode = false;
  ...
  case MODE_INPUTS: {
    ...
    if (_page == THR_CURVE) {
      ...
      if (focusedItem == 2)
        ...
        Model.throttlePts[_thisPt] = incDecOnUpDown(
            Model.throttlePts[_thisPt], -100, 100, NOWRAP, PRESSED_OR_HELD);
    ...
    if (_page == SLOW1) {
      ...
      if (focusedItem == 2)
        Model.slow1Up =
            incDecOnUpDown(Model.slow1Up, 0, 50, NOWRAP, PRESSED_OR_HELD);
      else if (focusedItem == 3)
        Model.slow1Down =
            incDecOnUpDown(Model.slow1Down, 0, 50, NOWRAP, PRESSED_OR_HELD);
      else if (focusedItem == 4)
        Model.slow1Src = incDecOnUpDown(Model.slow1Src, IDX_SWA, IDX_SWF,
                                        NOWRAP, PRESSED_OR_HELD);
    }
  ...
  case MODE_MIXER: {
    ...
    if (focusedItem == 1) // Change to another mixer slot
      thisMixNum =
          incDecOnUpDown(thisMixNum, 0, NUM_MIXSLOTS - 1, WRAP, SLOW_CHANGE);
      ...
      Model.mixIn1[thisMixNum] = incDecOnUpDown(
          Model.mixIn1[thisMixNum], 0, NUM_MIXSOURCES - 1, NOWRAP, SLOW_CHANGE);
    else if (focusedItem == 4) // adjust weight 1
      Model.mixIn1Weight[thisMixNum] = incDecOnUpDown(
          Model.mixIn1Weight[thisMixNum], -100, 100, NOWRAP, PRESSED_OR_HELD);
    else if (focusedItem == 5) // adjust offset 1
      Model.mixIn1Offset[thisMixNum] = incDecOnUpDown(
          Model.mixIn1Offset[thisMixNum], -100, 100, NOWRAP, PRESSED_OR_HELD);
    else if (focusedItem == 6) // adjust differential 1
      Model.mixIn1Diff[thisMixNum] = incDecOnUpDown(
          Model.mixIn1Diff[thisMixNum], -100, 100, NOWRAP, PRESSED_OR_HELD);
    else if (focusedItem == 7) // change operator
    {
      uint8_t _mixOper = Model.mixOper_N_Switch[thisMixNum] >> 6;
      _mixOper =
          incDecOnUpDown(_mixOper, 0, NUM_MIXOPERATORS - 1, WRAP, PRESSED_ONLY);
  ...
  case POPUP_MOVE_MIX: {
    ...
    destMixNum =
        incDecOnUpDown(destMixNum, 0, NUM_MIXSLOTS - 1, WRAP, SLOW_CHANGE);
  ...
  case MODE_OUTPUTS: {
    ...
    if (focusedItem == 1)
      _selectedChannel = incDecOnUpDown(
          _selectedChannel, 0, NUM_PRP_CHANNLES - 1, WRAP, SLOW_CHANGE);
      ...
      Model.subtrim[_selectedChannel] = incDecOnUpDown(
          Model.subtrim[_selectedChannel], -20, 20, NOWRAP, SLOW_CHANGE);
    else if (focusedItem == 4)
      Model.failsafe[_selectedChannel] = incDecOnUpDown(
          Model.failsafe[_selectedChannel], -101, 100, NOWRAP, PRESSED_OR_HELD);
    else if (focusedItem == 5)
      Model.endpointL[_selectedChannel] = incDecOnUpDown(
          Model.endpointL[_selectedChannel], -100, 0, NOWRAP, PRESSED_OR_HELD);
    else if (focusedItem == 6)
      Model.endpointR[_selectedChannel] = incDecOnUpDown(
          Model.endpointR[_selectedChannel], 0, 100, NOWRAP, PRESSED_OR_HELD);
  ...
  case MODE_SYSTEM: {
    ...
    if (focusedItem == 1)
      Sys.rfOutputEnabled =
          incDecOnUpDown(Sys.rfOutputEnabled, 0, 1, WRAP, PRESSED_ONLY);
    else if (focusedItem == 2)
      Sys.rfPower =
          incDecOnUpDown(Sys.rfPower, 0, RFPOWER_LAST, NOWRAP, PRESSED_ONLY);
    else if (focusedItem == 3)
      Sys.backlightMode = incDecOnUpDown(Sys.backlightMode, 0, BACKLIGHT_LAST,
                                         NOWRAP, PRESSED_ONLY);
    else if (focusedItem == 4)
      Sys.soundMode =
          incDecOnUpDown(Sys.soundMode, 0, SOUND_LAST, NOWRAP, PRESSED_ONLY);
    else if (focusedItem == 5)
      Sys.inactivityMinutes =
          incDecOnUpDown(Sys.inactivityMinutes, 0, 20, NOWRAP, SLOW_CHANGE);
  ...
  case MODE_TELEMETRY: {
    ...
    if (focusedItem == 2)
      Sys.telemAlarmEnabled =
          incDecOnUpDown(Sys.telemAlarmEnabled, 0, 1, WRAP, PRESSED_ONLY);
    else if (focusedItem == 3)
      Sys.telemVoltsOnHomeScreen =
          incDecOnUpDown(Sys.telemVoltsOnHomeScreen, 0, 1, WRAP, PRESSED_ONLY);
    else if (focusedItem == 4)
      Model.telemVoltsThresh =
          incDecOnUpDown(Model.telemVoltsThresh, 0, 2500, NOWRAP, FAST_CHANGE);
  ...
  case MODE_CALIB: {
    ...
    if (calibStage == STICKS_MOVE) {
      ...
      drawCursor(51, 34);
      Sys.deadZonePerc =
          incDecOnUpDown(Sys.deadZonePerc, 0, 15, NOWRAP, PRESSED_OR_HELD);
...
int incDecOnUpDown(int _val, int _lowerLimit, int _upperLimit, bool _enableWrap,
  ...
  if (_heldBtn > 0 && (millis() - buttonStartTime > (LONGPRESSTIME + 3000UL)) &&
      _state == FAST_CHANGE)
    delta = 10;
