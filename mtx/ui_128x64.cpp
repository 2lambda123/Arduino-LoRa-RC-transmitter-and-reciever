...
#include "LCDKS0108.h"
LCDKS0108 display =
    LCDKS0108(PIN_KS_RS, PIN_KS_EN, PIN_KS_CS1, PIN_KS_CS2, PIN_LATCH);
#elif defined(DISPLAY_CGM12864G)
#include "LCDCGM12864G_595.h"
LCDCGM12864G_595 display =
    LCDCGM12864G_595(PIN_CGM_RSEL, PIN_CGM_RD, PIN_CGM_RST, PIN_LATCH);
#endif

enum { WRAP = true, NOWRAP = false };
enum { INCDEC_PRESSED_ONLY = 0, INCDEC_SLOW, INCDEC_NORMAL, INCDEC_FAST };
...
void makeToast(const __FlashStringHelper *text, uint16_t _duration,
               uint16_t _delay);
void drawToast();
...
int incDecOnUpDown(int _val, int _lowerLimit, int _upperLimit, bool _enableWrap,
                   uint8_t _state);
...
const char *const timerMenuA[] PROGMEM = {tmrStr0, tmrStr2, tmrStr3, tmrStr4};
const char *const timerMenuB[] PROGMEM = {tmrStr1, tmrStr2, tmrStr3, tmrStr4};
...
const char *const srcNames[] PROGMEM = {
    srcName0,  srcName1,  srcName2,  srcName3,  srcName4,  srcName5,
    srcName6,  srcName7,  srcName8,  srcName9,  srcName10, srcName11,
    srcName12, srcName13, srcName14, srcName15, srcName16, srcName17,
    srcName18, srcName19, srcName20, srcName21, srcName22, srcName23,
    srcName24, srcName25, srcName26, srcName27, srcName28, srcName29};
...
const char *const mixSwitchStr[] PROGMEM = {
    ...
    mxSwStr6,  mxSwStr7,  mxSwStr8,  mxSwStr9,  mxSwStr10, mxSwStr11,
    mxSwStr12, mxSwStr13, mxSwStr14, mxSwStr15, mxSwStr16};
...
const char *const soundModeStr[] PROGMEM = {soundModeStr0, soundModeStr1,
                                            soundModeStr2, soundModeStr3};
...
const char *const activeModelMenu[] PROGMEM = {modelRenameStr, modelCopyStr,
                                               modelResetStr};
#define NUM_ITEMS_INACTIVE_MODEL_MENU 2
const char *const inactiveModelMenu[] PROGMEM = {modelLoadStr, modelDeleteStr};
#define NUM_ITEMS_FREE_MODEL_MENU 1
const char *const freeModelMenu[] PROGMEM = {modelCreateStr};
...
const char *const backlightModeStr[] PROGMEM = {
    backlightModeStr0, backlightModeStr1, backlightModeStr2, backlightModeStr3,
    backlightModeStr4};
...
const char *const rfPowerStr[] PROGMEM = {rfPowerStr0, rfPowerStr1, rfPowerStr2,
                                          rfPowerStr3, rfPowerStr4};
...
const char *const tmrOperatorStr[] PROGMEM = {tmrOperatorStr0, tmrOperatorStr1,
                                              tmrOperatorStr2, tmrOperatorStr3};
...
const char *const funcgenStr[] PROGMEM = {funcgenStr0, funcgenStr1, funcgenStr2,
                                          funcgenStr3};
...
uint8_t focusedItem =
    1; // The item that currently has focus in MODE Screens or popups
...
void startStickCalibration() { changeToScreen(MODE_CALIB); }
...
void handleStartupMenu() {
  ...
  while (1) {
    ...
    if (_selection == 1) {
      ...
      EEPROM.write(EE_INITFLAG_ADDR,
                   ~EEPROM.read(EE_INITFLAG_ADDR)); // clear flag
      return;                                       // exit
...
void handleMainUI() {
  ...
  if (Sys.inactivityMinutes > 0 &&
      ((millis() - inputsLastMoved) > (Sys.inactivityMinutes * 60000UL))) {
  ...
  if (Sys.telemAlarmEnabled && telem_volts != 0x0FFF) {
    ...
    if (_tWarnStarted &&
        ((thisLoopNum - _tWarnEntryLoopNum) % (5000 / fixedLoopTime) == 1))
  ...
  if (!stopwatchIsPaused) // run
    stopwatchElapsedTime =
        stopwatchLastElapsedTime + millis() - stopwatchLastPaused;
  else // pause
  {
  ...
  if (Model.timer1InitMins > 0) {
    ...
    if ((timer1ElapsedTime > _initMillis) &&
        timer1ElapsedTime < (_initMillis + 500) && !_alarmTriggered) {
  ...
  case HOME_SCREEN: {
    ...
    // This crude battery gauge doesn't indicate state of charge; only battery
    // voltage
    ...
    if (battState == BATTHEALTY) {
      int8_t numOfBars =
          (20L * (battVoltsNow - battVoltsMin)) / (battVoltsMax - battVoltsMin);
      if (numOfBars > 19)
        numOfBars = 19;
      if (numOfBars > lastNumOfBars &&
          numOfBars - lastNumOfBars < 2) // prevent jitter at boundaries
    ...
    if (homeScreenMode == TRIMMODE) {
      ...
      strlcpy_P(txtBuff,
                (char *)pgm_read_word(&(srcNames[IDX_AIL + selectedTrim])),
                sizeof(txtBuff));
  ...
  case MODE_TIMER_SETUP: {
    ...
    strlcpy_P(txtBuff,
              (char *)pgm_read_word(&(srcNames[Model.timer1ControlSrc])),
              sizeof(txtBuff));
    display.print(txtBuff);
    ...
    strlcpy_P(txtBuff,
              (char *)pgm_read_word(&(tmrOperatorStr[Model.timer1Operator])),
              sizeof(txtBuff));
    display.print(txtBuff);
    ...
    if (focusedItem == 1) {
      Model.timer1ControlSrc = incDecOnUpDown(
          Model.timer1ControlSrc, 0, NUM_MIXSOURCES - 1, NOWRAP, INCDEC_SLOW);
      // Validate sources. Allowed sources are raw sticks, knob, switches,
      // channels, virtuals
      while (Model.timer1ControlSrc == IDX_100PERC ||
             Model.timer1ControlSrc == IDX_FUNCGEN ||
             (Model.timer1ControlSrc >= IDX_SLOW1 &&
              Model.timer1ControlSrc <= IDX_RUD)) {
        Model.timer1ControlSrc = incDecOnUpDown(
            Model.timer1ControlSrc, 0, NUM_MIXSOURCES - 1, NOWRAP, INCDEC_SLOW);
      }
    } else if (focusedItem == 2)
      Model.timer1Operator =
          incDecOnUpDown(Model.timer1Operator, 0, NUM_TIMER_OPERATORS - 1,
                         NOWRAP, INCDEC_SLOW);
    else if (focusedItem == 3)
      Model.timer1Value =
          incDecOnUpDown(Model.timer1Value, -100, 100, NOWRAP, INCDEC_NORMAL);
    else if (focusedItem == 4)
      Model.timer1InitMins =
          incDecOnUpDown(Model.timer1InitMins, 0, 240, NOWRAP, INCDEC_NORMAL);
  ...
  case MAIN_MENU: {
    ...
    //------ handle navigation
    isEditMode = true;
    menuHighlightedItem = incDecOnUpDown(menuHighlightedItem, _numMenuItems, 1,
                                         WRAP, INCDEC_SLOW);
    if (menuHighlightedItem < menuTopItem ||
        menuHighlightedItem >= (menuTopItem + 4))
      menuTopItem =
          incDecOnUpDown(menuTopItem, _numMenuItems - 3, 1, WRAP, INCDEC_SLOW);
    isEditMode = false;

    //------ show heading
    strlcpy_P(txtBuff, (char *)pgm_read_word(&mainMenu[0]), sizeof(txtBuff));
    uint8_t txtWidthPix = strlen(txtBuff) * 6;
    uint8_t headingX_offset =
        (display.width() - txtWidthPix) / 2; // middle align heading
    ...
    {
      strlcpy_P(txtBuff, (char *)pgm_read_word(&mainMenu[menuTopItem + i]),
                sizeof(txtBuff));
    ...
    if (_numMenuItems > 4) {
      ...
      uint8_t barSize =
          ((int16_t)viewPortHeight * viewPortHeight) / contentHeight;
  ...
  case MODE_MODEL: {
    ...
    if (!_viewInitialised) {
      ...
      if (Sys.activeModel > 6)
        _top = Sys.activeModel - 5;
      else
        _top = 1;
    ...
    for (uint8_t i = 1; i <= 6 && i <= maxNumOfModels; i++) {
      ...
      {
        if (_mdlNo < 10)
          display.fillRect(30, 9 * i, 7, 9, BLACK);
        else
          display.fillRect(30, 9 * i, 14, 9, BLACK);
        display.setTextColor(WHITE);
      }
  ...
  case POPUP_RENAME_MODEL: {
    ...
    if (thisChar == 32)
      ...
      thisChar = 90 - thisChar;
    else if (thisChar <= 122)
      thisChar -= 70;
    ...
    //--- clear model name ----
    if (charPos == 0 && heldButton == SELECT_KEY &&
        millis() - buttonStartTime >= 1500) {
      setDefaultModelName();
      heldButton = 0;
    }
  ...
  case MODE_INPUTS: {
    ...
    if (_page == AIL_CURVE || _page == ELE_CURVE || _page == RUD_CURVE) {
      ...
      if (_page == RUD_CURVE)
        strlcpy_P(txtBuff, (char *)pgm_read_word(&(srcNames[IDX_RUD])),
                  sizeof(txtBuff));
      else
        strlcpy_P(txtBuff, (char *)pgm_read_word(&(srcNames[IDX_AIL + _page])),
                  sizeof(txtBuff));
      display.print(txtBuff);
    ...
    if (_page == THR_CURVE) {
      ...
      if (focusedItem == 2)
        ...
        Model.throttlePts[_thisPt] = incDecOnUpDown(
            Model.throttlePts[_thisPt], -100, 100, NOWRAP, INCDEC_NORMAL);

      //-----draw text
      display.setCursor(8, 11);
      strlcpy_P(txtBuff, (char *)pgm_read_word(&(srcNames[IDX_THRTL_CURV])),
                sizeof(txtBuff));
      display.print(txtBuff);
      display.drawHLine(8, 19, strlen(txtBuff) * 6, BLACK);
      ...
      // Interpolate and draw points. We use x cordinate to estimate
      // corresponding y cordinate Actual plot area is 50x50.
      int xpts[5] = {-500, -250, 0, 250, 500};
      int ypts[5];
      for (uint8_t i = 0; i < 5; i++)
        ypts[i] = Model.throttlePts[i] * 5;

      for (int xval = -25; xval <= 25;
           xval++) // 50x50 grid so first point is -25
      {
    ...
    if (_page == SLOW1) {
      ...
      display.setCursor(0, 40);
      display.print(F("Src:   "));
      strlcpy_P(txtBuff, (char *)pgm_read_word(&(srcNames[Model.slow1Src])),
                sizeof(txtBuff));
      display.print(txtBuff);

      if (focusedItem == 2)
        Model.slow1Up =
            incDecOnUpDown(Model.slow1Up, 0, 50, NOWRAP, INCDEC_NORMAL);
      else if (focusedItem == 3)
        Model.slow1Down =
            incDecOnUpDown(Model.slow1Down, 0, 50, NOWRAP, INCDEC_NORMAL);
      else if (focusedItem == 4)
        Model.slow1Src = incDecOnUpDown(Model.slow1Src, IDX_SWA, IDX_SWF,
                                        NOWRAP, INCDEC_SLOW);
    }
    ...
    if (_page == FUNCGEN) {
      ...
      strlcpy_P(txtBuff,
                (char *)pgm_read_word(&(funcgenStr[Model.funcgenWaveform])),
                sizeof(txtBuff));
      display.print(txtBuff);
      ...
      if (focusedItem == 2)
        Model.funcgenWaveform =
            incDecOnUpDown(Model.funcgenWaveform, 0, NUM_FUNC_WAVEFORMS - 1,
                           WRAP, INCDEC_SLOW);
      else if (focusedItem == 3)
        Model.funcgenPeriod =
            incDecOnUpDown(Model.funcgenPeriod, 5, 200, NOWRAP, INCDEC_NORMAL);
    }
    ...
    if (_page == RAW_INPUTS) {
      ...
      int _stickVal[5] = {rollIn, pitchIn, throttleIn, yawIn,
                          knobIn}; // order as in source names
      for (uint8_t i = 0; i < 5; i++) {
        display.setCursor(11, 21 + i * 9);
        strlcpy_P(txtBuff, (char *)pgm_read_word(&(srcNames[IDX_ROLL + i])),
                  sizeof(txtBuff));
        display.print(txtBuff);
      ...
      // show switches
      uint8_t _swState[6] = {swAEngaged, swBEngaged, swCState,
                             swDEngaged, swEEngaged, swFEngaged};
      for (uint8_t i = 0; i < 6; i++) {
        uint8_t _ycord = 12 + i * 9;
        display.setCursor(71, _ycord);
        strlcpy_P(txtBuff, (char *)pgm_read_word(&(srcNames[IDX_SWA + i])),
                  sizeof(txtBuff));
        display.print(txtBuff);
  ...
  case MODE_MIXER: {
    ...
    strlcpy_P(txtBuff, (char *)pgm_read_word(&(srcNames[_outNameIndex])),
              sizeof(txtBuff));
    display.print(txtBuff);
    ...
    for (uint8_t i = 0; i < 2; i++) {
      ...
      if (_inName[i] == IDX_SLOW1) {
        ...
        display.drawBitmap(display.getCursorX(), display.getCursorY(),
                           asterisk_small, 3, 3, 1);
      } else {
        strlcpy_P(txtBuff, (char *)pgm_read_word(&(srcNames[_inName[i]])),
                  sizeof(txtBuff));
        display.print(txtBuff);
      }
    }
    ...
    strlcpy_P(txtBuff, (char *)pgm_read_word(&(mixSwitchStr[_idx])),
              sizeof(txtBuff));
    display.print(txtBuff);
    ...
    if (focusedItem == 1) // Change to another mixer slot
      thisMixNum =
          incDecOnUpDown(thisMixNum, 0, NUM_MIXSLOTS - 1, WRAP, INCDEC_SLOW);
    else if (focusedItem == 2) // change output
      Model.mixOut[thisMixNum] =
          incDecOnUpDown(Model.mixOut[thisMixNum], IDX_NONE, NUM_MIXSOURCES - 1,
                         NOWRAP, INCDEC_SLOW);
    else if (focusedItem == 3) // change input 1
      Model.mixIn1[thisMixNum] = incDecOnUpDown(
          Model.mixIn1[thisMixNum], 0, NUM_MIXSOURCES - 1, NOWRAP, INCDEC_SLOW);
    else if (focusedItem == 4) // adjust weight 1
      Model.mixIn1Weight[thisMixNum] = incDecOnUpDown(
          Model.mixIn1Weight[thisMixNum], -100, 100, NOWRAP, INCDEC_NORMAL);
    else if (focusedItem == 5) // adjust offset 1
      Model.mixIn1Offset[thisMixNum] = incDecOnUpDown(
          Model.mixIn1Offset[thisMixNum], -100, 100, NOWRAP, INCDEC_NORMAL);
    else if (focusedItem == 6) // adjust differential 1
      Model.mixIn1Diff[thisMixNum] = incDecOnUpDown(
          Model.mixIn1Diff[thisMixNum], -100, 100, NOWRAP, INCDEC_NORMAL);
    else if (focusedItem == 7) // change operator
    {
      uint8_t _mixOper = Model.mixOper_N_Switch[thisMixNum] >> 6;
      _mixOper =
          incDecOnUpDown(_mixOper, 0, NUM_MIXOPERATORS - 1, WRAP, INCDEC_SLOW);
      ...
      Model.mixIn2[thisMixNum] = incDecOnUpDown(
          Model.mixIn2[thisMixNum], 0, NUM_MIXSOURCES - 1, NOWRAP, INCDEC_SLOW);
    else if (focusedItem == 9) // adjust weight 2
      Model.mixIn2Weight[thisMixNum] = incDecOnUpDown(
          Model.mixIn2Weight[thisMixNum], -100, 100, NOWRAP, INCDEC_NORMAL);
    else if (focusedItem == 10) // adjust offset 2
      Model.mixIn2Offset[thisMixNum] = incDecOnUpDown(
          Model.mixIn2Offset[thisMixNum], -100, 100, NOWRAP, INCDEC_NORMAL);
    else if (focusedItem == 11) // adjust differential 2
      Model.mixIn2Diff[thisMixNum] = incDecOnUpDown(
          Model.mixIn2Diff[thisMixNum], -100, 100, NOWRAP, INCDEC_NORMAL);
    else if (focusedItem == 12) // change switch
    {
      uint8_t _mixSw = Model.mixOper_N_Switch[thisMixNum] & 0x3F;
      _mixSw =
          incDecOnUpDown(_mixSw, 0, NUM_MIXSWITCHES - 1, NOWRAP, INCDEC_SLOW);
  ...
  case POPUP_MOVE_MIX: {
    ...
    destMixNum =
        incDecOnUpDown(destMixNum, 0, NUM_MIXSLOTS - 1, WRAP, INCDEC_SLOW);
    ...
    if (clickedButton == SELECT_KEY) {
      ...
      if (newPostn < oldPostn) {
        while (thisPostn > newPostn) {
          ...
          Model.mixOper_N_Switch[thisPostn] =
              Model.mixOper_N_Switch[thisPostn - 1];
  ...
  case MODE_OUTPUTS: {
    ...
    if (focusedItem == 1)
      _selectedChannel = incDecOnUpDown(
          _selectedChannel, 0, NUM_PRP_CHANNLES - 1, WRAP, INCDEC_SLOW);
    else if (focusedItem == 2 && isEditMode) {
      if (pressedButton == UP_KEY || pressedButton == DOWN_KEY)
        Model.reverse ^= (uint16_t)1 << _selectedChannel; // toggle bit
    } else if (focusedItem == 3)
      Model.subtrim[_selectedChannel] = incDecOnUpDown(
          Model.subtrim[_selectedChannel], -20, 20, NOWRAP, INCDEC_SLOW);
    else if (focusedItem == 4)
      Model.failsafe[_selectedChannel] = incDecOnUpDown(
          Model.failsafe[_selectedChannel], -101, 100, NOWRAP, INCDEC_NORMAL);
    else if (focusedItem == 5)
      Model.endpointL[_selectedChannel] = incDecOnUpDown(
          Model.endpointL[_selectedChannel], -100, 0, NOWRAP, INCDEC_NORMAL);
    else if (focusedItem == 6)
      Model.endpointR[_selectedChannel] = incDecOnUpDown(
          Model.endpointR[_selectedChannel], 0, 100, NOWRAP, INCDEC_NORMAL);
  ...
  case MODE_SYSTEM: {
    ...
    strlcpy_P(txtBuff, (char *)pgm_read_word(&(rfPowerStr[Sys.rfPower])),
              sizeof(txtBuff));
    display.print(txtBuff);
    ...
    strlcpy_P(txtBuff,
              (char *)pgm_read_word(&(backlightModeStr[Sys.backlightMode])),
              sizeof(txtBuff));
    display.print(txtBuff);

    display.setCursor(0, 37);
    display.print(F("Sounds  :  "));
    strlcpy_P(txtBuff, (char *)pgm_read_word(&(soundModeStr[Sys.soundMode])),
              sizeof(txtBuff));
    display.print(txtBuff);
    ...
    if (focusedItem == 1)
      Sys.rfOutputEnabled =
          incDecOnUpDown(Sys.rfOutputEnabled, 0, 1, WRAP, INCDEC_PRESSED_ONLY);
    else if (focusedItem == 2)
      Sys.rfPower = incDecOnUpDown(Sys.rfPower, 0, RFPOWER_LAST, NOWRAP,
                                   INCDEC_PRESSED_ONLY);
    else if (focusedItem == 3)
      Sys.backlightMode = incDecOnUpDown(Sys.backlightMode, 0, BACKLIGHT_LAST,
                                         NOWRAP, INCDEC_PRESSED_ONLY);
    else if (focusedItem == 4)
      Sys.soundMode = incDecOnUpDown(Sys.soundMode, 0, SOUND_LAST, NOWRAP,
                                     INCDEC_PRESSED_ONLY);
    else if (focusedItem == 5)
      Sys.inactivityMinutes =
          incDecOnUpDown(Sys.inactivityMinutes, 0, 20, NOWRAP, INCDEC_SLOW);
  ...
  case MODE_TELEMETRY: {
    ...
    if (focusedItem == 2)
      Sys.telemAlarmEnabled = incDecOnUpDown(Sys.telemAlarmEnabled, 0, 1, WRAP,
                                             INCDEC_PRESSED_ONLY);
    else if (focusedItem == 3)
      Sys.telemVoltsOnHomeScreen = incDecOnUpDown(Sys.telemVoltsOnHomeScreen, 0,
                                                  1, WRAP, INCDEC_PRESSED_ONLY);
    else if (focusedItem == 4)
      Model.telemVoltsThresh =
          incDecOnUpDown(Model.telemVoltsThresh, 0, 2500, NOWRAP, INCDEC_FAST);
  ...
  case MODE_RECEIVER: {
    ...
    if (_state != _VIEWING_CONFIG) {
      ...
      toggleEditModeOnSelectClicked();
      if (focusedItem <= 5)
        drawCursor(28, focusedItem * 9);
      else if (focusedItem < 10)
        drawCursor(94, (focusedItem - 5) * 9);
      else if (focusedItem == 10)
        drawCursor(85, 56);
      ...
      if (focusedItem < 10) {
        ...
        outputChConfig[_idx] =
            incDecOnUpDown(outputChConfig[_idx], 0, maxOutputChConfig[_idx],
                           WRAP, INCDEC_SLOW);
  ...
  case MODE_CALIB: {
    ...
    if (calibStage == STICKS_MOVE) {
      ...
      if (clickedButton == SELECT_KEY) {
        // Add slight deadband(about 1.5%) at each stick ends to stabilise
        // readings at ends For a range of 0 to 5V, min max are 0.07V and 4.92V
      ...
      drawCursor(51, 34);
      Sys.deadZonePerc =
          incDecOnUpDown(Sys.deadZonePerc, 0, 15, NOWRAP, INCDEC_NORMAL);
...
int incDecOnUpDown(int _val, int _lowerLimit, int _upperLimit, bool _enableWrap,
                   uint8_t _state) {
  // Increments/decrements the passed value between the specified limits
  // inclusive. If wrap is enabled, wraps around when either limit is reached.
  ...
  if ((_state == INCDEC_SLOW && thisLoopNum % (125 / fixedLoopTime) == 1) ||
      _state == INCDEC_NORMAL || _state == INCDEC_FAST) {
    _heldBtn = heldButton;
  }
  ...
  if (_state >= INCDEC_NORMAL && _heldBtn > 0) {
    if (millis() - buttonStartTime > (LONGPRESSTIME + 1000UL))
      delta = 2;
    if (millis() - buttonStartTime > (LONGPRESSTIME + 3000UL) &&
        _state == INCDEC_FAST)
      delta = 10;
  }
...
void drawCursor(uint8_t _xpos, uint8_t _ypos) {
  ...
  {
    if ((millis() - buttonReleaseTime) % 1000 < 500 || buttonCode == UP_KEY ||
        buttonCode == DOWN_KEY)
...
void drawHeader(const char *str) {
  ...
  uint8_t headingX_offset =
      (display.width() - _txtWidthPix) / 2; // middle align heading
...
void drawFullScreenMsg(const char *str) {
  ...
  uint8_t y_offset =
      (display.height() - numTextLines * 9) / 2; // 9 is line pitch
...
void drawToast() {
  ...
  if (_currTime >= toastStartTime && _currTime < toastExpireTime) {
    ...
    uint8_t _txtWidthPix =
        6 * strlen_P((const char *)toastText); //(const char*) casts
...
void drawPopupMenu(const char *const list[], int8_t _numItems) {
  ...
  uint8_t _yOffsetStr0 = (display.height() - (_numItems * 9)) /
                         2; // 9 is pitchsize between text lines
  _yOffsetStr0 += 1;
...
void drawLoadingAnimation(uint8_t xpos, uint8_t ypos, uint8_t _size) {
  ...
  static const uint8_t activeCells[] PROGMEM = {
      1,  2,  7, 2,  7, 11, 7, 11, 14, 11, 14, 13,
      14, 13, 8, 13, 8, 4,  8, 4,  1,  4,  1,  2,
  };

  const int frameTime = 50;
  uint8_t frameIdx =
      (millis() % (8 * frameTime)) / frameTime; // total of 8 frames
