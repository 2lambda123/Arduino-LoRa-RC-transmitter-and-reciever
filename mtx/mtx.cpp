...
void setup() {
  ...
  uint8_t eeInitFlag = crc8Maxim((uint8_t *)&Sys, sizeof(Sys)) ^
                       crc8Maxim((uint8_t *)&Model, sizeof(Model));

  bool formatEE = false;

  /// Check signature. If its not matching, then assume it's a fresh mcu and
  /// format the eeprom
...
void checkBattery() {
  ...
  // As the implementation here uses integer math and the technique is
  // recursive, there is loss of precision but this doesn't matter much here.
  const int smoothFactor = 5; //>0,<100
  long sample = ((long)analogRead(PIN_BATTVOLTS) * battVfactor) / 100;
  long battV =
      ((sample * smoothFactor) + ((100 - smoothFactor) * (long)battVoltsNow)) /
      100;
