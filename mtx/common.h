...
//---- Output channels --------------------
extern int channelOut[NUM_PRP_CHANNLES]; // Proportional Channels. Centered at
                                         // 0, range is -500 to 500.
extern int8_t
    mixerChOutGraphVals[NUM_PRP_CHANNLES]; // for graphing raw mixer output for
                                           // channels. range -100 to 100

//---- Sticks -----------------------------
extern int rollIn, pitchIn, throttleIn, yawIn,
    knobIn; // Scaled stick values, range -500 to 500
...
extern uint8_t clickedButton; // triggered when the button is released before
                              // heldButton event
...
//---- Telemetry --------------------------
extern uint16_t
    telem_volts; // in 10mV, sent by receiver with 12bits.  0x0FFF "No data"
...
the time taken by radio module to transmit the entire packet or else the window
is missed resulting in much less throughput. It should also be atleast the time
taken to draw the UI, else or else the timing becomes inconsistent*/
...
#define NUM_MIXSLOTS 12 // More slots results into less models and more ram
                        // usage
...
typedef struct {
  ...
  uint8_t mixOper_N_Switch[NUM_MIXSLOTS]; // upper 2 bits for operator, other 6
                                          // bits for the switch
