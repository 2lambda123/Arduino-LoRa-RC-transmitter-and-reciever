/*
  ...
  ---------------------------------------------------------------------------------------------
  FILE_SIGNATURE | EE_INITFLAG | System Data | 1st Model Data | 2nd Model Data |
  nth Model Data
  ---------------------------------------------------------------------------------------------
*/
...
void eeReadSysConfig() { EEPROM.get(eeSysDataStartAddress, Sys); }

//==================================================================================================

void eeSaveSysConfig() { EEPROM.put(eeSysDataStartAddress, Sys); }
...
void eeCopyModelName(char *_buff, uint8_t _mdlNo) {
  ...
  uint16_t _mdlOffset_ = getModelDataOffsetAddr(_mdlNo);
  for (uint8_t i = 0;
       i < (sizeof(Model.modelName) / sizeof(Model.modelName[0])); i++) {
...
void eeDeleteModel(uint8_t _mdlNo) {
  ...
  EEPROM.update(eeModelDataStartAddress + _mdlOffset_ + len - 1,
                '\0'); // write termination char
}
