...
int deadzoneAndMap(int _input, int _minVal, int _centerVal, int _maxVal,
                   int _deadzn, int _mapMin, int _mapMax);
int linearInterpolate(int xValues[], int yValues[], uint8_t numValues,
                      int pointX);
int applySlow(int _currentVal, int _targetVal, uint16_t _riseTime,
              uint16_t _fallTime);
...
void readSwitchesAndButtons() {
  ...
  // Prevent phantom keys. (These arise if 2 or more buttons are pressed
  // simultanouesly (missing some diodes). This situation can falsely report
  // switches and lead to loss of control :(
  ...
  uint8_t switchesSum =
      swAEngaged + swBEngaged + swCState + swDEngaged + swEEngaged + swFEngaged;
...
void determineButtonEvent() {
  /*
  Modifies the pressedButton, clickedButton and heldButton variables,
  buttonStartTime and buttonReleaseTime. Events
  - pressedButton is triggered once when the button goes down
  - clickedButton is triggered when the button is released before heldButton
  event
  ...
  */
  ...
  if (buttonActive && !longPressActive &&
      (millis() - buttonStartTime > LONGPRESSTIME)) {
...
void readSticks() {
  ...
  rollIn = deadzoneAndMap(rollIn, Sys.rollMin, Sys.rollCenterVal, Sys.rollMax,
                          Sys.deadZonePerc, -500, 500);
  pitchIn = deadzoneAndMap(pitchIn, Sys.pitchMin, Sys.pitchCenterVal,
                           Sys.pitchMax, Sys.deadZonePerc, -500, 500);
  yawIn = deadzoneAndMap(yawIn, Sys.yawMin, Sys.yawCenterVal, Sys.yawMax,
                         Sys.deadZonePerc, -500, 500);
      ...
      (knobIn < 0 && _knobRegion == _POS_SIDE)) // crossed center
  {
    ...
    _knobRegion = _POS_SIDE;
  else if (knobIn < -25)
    _knobRegion = _NEG_SIDE;
...
void computeChannelOutputs() {
  ...
  mixSources[IDX_SWC] =
      (swCState == SWLOWERPOS) ? 500 : ((swCState == SWUPPERPOS) ? -500 : 0);
  ...
  for (uint8_t i = 0; i < 3; i++) {
    if (!swBEngaged || ((Model.dualRate >> i) & 1) == 0)
      mixSources[_idx[i]] =
          calcRateExpo(_stickIn[i], Model.rateNormal[i], Model.expoNormal[i]);
    else
      mixSources[_idx[i]] =
          calcRateExpo(_stickIn[i], Model.rateSport[i], Model.expoSport[i]);
  }
  ...
  mixSources[IDX_THRTL_CURV] =
      linearInterpolate(xpoints, ypoints, 5, throttleIn);

  ///--Mix source Slow1
  static int _valueNow = mixSources[IDX_SLOW1];
  mixSources[IDX_SLOW1] = applySlow(_valueNow, mixSources[Model.slow1Src],
                                    Model.slow1Up * 100, Model.slow1Down * 100);
  ...
  for (uint8_t _mixNum = 0; _mixNum < NUM_MIXSLOTS; _mixNum++) {
    ...
    if (Model.mixIn1[_mixNum] != IDX_NONE) {
      _operand1 = weightAndOffset(
          mixSources[Model.mixIn1[_mixNum]], Model.mixIn1Weight[_mixNum],
          Model.mixIn1Offset[_mixNum], Model.mixIn1Diff[_mixNum]);

      // Handle trim here so that differential works as expected
      if (Model.mixIn1[_mixNum] >= IDX_AIL &&
          Model.mixIn1[_mixNum] <= IDX_RUD) {
    ...
    if (Model.mixIn2[_mixNum] != IDX_NONE) {
      _operand2 = weightAndOffset(
          mixSources[Model.mixIn2[_mixNum]], Model.mixIn2Weight[_mixNum],
          Model.mixIn2Offset[_mixNum], Model.mixIn2Diff[_mixNum]);

      // Handle trim here so that differential works as expected
      if (Model.mixIn2[_mixNum] >= IDX_AIL &&
          Model.mixIn2[_mixNum] <= IDX_RUD) {
    ...
    if (Model.mixOut[_mixNum] <
        NUM_MIXSOURCES) // protects from potential overruns
  ...
  for (uint8_t i = 0; i < NUM_PRP_CHANNLES; i++) {
    ...
    //---Endpoints
    channelOut[i] = constrain(channelOut[i], 5 * Model.endpointL[i],
                              5 * Model.endpointR[i]);
  }

  /// EVALUATE TIMER1
  if (Model.timer1ControlSrc >= IDX_CH1 &&
      Model.timer1ControlSrc < (IDX_CH1 + NUM_PRP_CHANNLES))
...
int deadzoneAndMap(int _input, int _minVal, int _centerVal, int _maxVal,
  ...
  _ddZnTmp /= 100;
  int _ddZnVal =
      int(_ddZnTmp) / 2; // divide by 2 as we apply deadzone about center
  int _mapCenter = (_mapMin / 2) + (_mapMax / 2);
  // add dead zone and map the input
  int _output;
  if (_input > _centerVal + _ddZnVal)
    _output =
        map(_input, _centerVal + _ddZnVal, _maxVal, _mapCenter + 1, _mapMax);
  else if (_input < _centerVal - _ddZnVal)
    _output =
        map(_input, _minVal, _centerVal - _ddZnVal, _mapMin, _mapCenter - 1);
  else
    _output = _mapCenter;
...
int calcRateExpo(int _input, int _rate, int _expo) {
  /* This function is for applying rate and cubic 'expo' to aileron, elevator
     and rudder channels. Ranges: _input  -500 to 500, _rate   0 to 100 _expo
     -100 to 100. 0 is linear
  */
  ...
  /**
    ...
    For our implementation, we only use the range 0 < k < 1 and simply 'invert'
    the result if positive expo is specified. We need to scale up this equation
    to avoid floating point maths. After calculation, we scale back the result.
    Thus the modified equation taking into account the range of our parameters
    (to ensure no overflow) is as follows. y = ( (k/100 * x/500)  +  ((1 -
    k/100)*((x*x*x)/(500*500*500))) ) * 500 This simplifies to y =  (((250000*k
    + x*x*(100-k)) / 250000 ) * x) / 100 We also work with only positive x
  */
...
bool mixSwitchIsActive(uint8_t _mixNum) {
  ...
  case SW_NONE:
    rslt = true;
    break; // SW_NONE is always active
  ...
  case SWC_UP:
    rslt = (swCState == SWUPPERPOS);
    break;
  case SWC_MID:
    rslt = (swCState == SWMIDPOS);
    break;
  case SWC_DOWN:
    rslt = (swCState == SWLOWERPOS);
    break;
...
static const uint8_t sineForm[] PROGMEM = {
    100, 106, 113, 119, 125, 131, 137, 143, 148, 154, 159, 164, 168, 173, 177,
    181, 184, 188, 190, 193, 195, 197, 198, 199, 200, 200, 200, 199, 198, 197,
    195, 193, 190, 188, 184, 181, 177, 173, 168, 164, 159, 154, 148, 143, 137,
    131, 125, 119, 113, 106, 100, 94,  87,  81,  75,  69,  63,  57,  52,  46,
    41,  36,  32,  27,  23,  19,  16,  12,  10,  7,   5,   3,   2,   1,   0,
    0,   0,   1,   2,   3,   5,   7,   10,  12,  16,  19,  23,  27,  32,  36,
    41,  46,  52,  57,  63,  69,  75,  81,  87,  94,
};

int generateWaveform() {
  // When the period is being adjusted there is a sudden discontinuity in the
  // result that causes the servo to jerk. We thus need a way to smoothly
  // transition to the new period. One approach is to wait for peak or zero
  // crossing before applying the new period. However this isn't ideal if the
  // period is long. The better approach is to add some calculated offset to
  // millis so that we maintain the next ratio of timeInstance/period upon
  // change. To get the offset the equation used is (newTimeInstance +
  // offset)/period = nextRatio
  ...
  int nextRatio =
      (timeInstance * 1000L) /
      oldPeriod; // scalingfactor 1000 just to avoid floating point math
  ...
  case FUNC_SINE: {
    ...
    int index = (timeInstance * 10000L) /
                period; /*upper 2 digits are the actual index in the LUT,
the lower 2 digits are the fractional part used for linear interpolation*/
    ...
    result =
        valL + (((valR - valL) * (index % 100)) / 100); // 100 is scaling factor
  } break;
